<template>
  <div class="register-box">
    ...
    <div v-if="responses[0] === 'User created'">
      <div class="result-box alert alert-success" v-for="info in responses">
        <p>{{ info }}</p>
        <p> Zaloguj się: <router-link to="/login">kliknij</router-link> </p>
      </div>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    validateRegisterForm() {
      const apiLink = 'http://localhost:3000/api/user/register';
      this.responses = [];
      let validForm = true;
